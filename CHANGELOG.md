...
### [0.0.6] - 2025-04-27
- Added support for uploading `userpics` into user profiles via the API.
...
### [0.0.5] - 2025-04-20
- Added role-based `auth` with `reader`, `writer`, `editor`, and `admin`.
- Added `LRU` cache for efficient loading of static files with limits.
- Added `html` directory to serve static content related to the frontend.
- Added routers and schemas for `token`, `user`, `secret`, and `lock`.
- Added `token retrieve` and `token invalidate` functionality.

### [0.0.4] - 2025-04-06
- Added `timed` decorator to log async function execution duration.
...
- Added `repository` that combines entity and cache management logic.
...
- Added `context` module to provide values passing in the application.
- Added `encrypt_helper` module to provide encryption and hashing.
