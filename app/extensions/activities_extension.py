...
ACTIVITY_USER_SELECT_ENABLED = True
...
class Activity(Base):
    ...
    def __init__(self, current_user: User, entity: DeclarativeBase,
                 operation_type: OperationType):
        self.user_id = current_user.id
        self.operation_type = operation_type
        self.entity_tablename = entity.__tablename__
        self.entity_id = entity.id
        self.entity_dict = self.extract_entity_dict(entity)

    def extract_entity_dict(self, entity: DeclarativeBase) -> json:
        return {x: repr(entity.__dict__[x])
                if x not in OBSCURED_KEYS else OBSCURED_VALUE
                for x in entity.__dict__ if not x.startswith("_")}
...
async def after_user_select(
    ...
    entity and performs any necessary post-processing actions.
    """
    if ACTIVITY_USER_SELECT_ENABLED:
        activity = Activity(current_user, user, OperationType.select)
