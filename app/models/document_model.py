...
original filename, document filename, size, MIME type, and summary.
It supports relationships with collections, users, tags, and document
metadata. The module also provides methods for handling encryption,
...
from app.helpers.encrypt_helper import (
    encrypt_str, decrypt_str, encrypt_int, decrypt_int, hash_str)
...
class Document(Base, MetaMixin):
    ...
    # value length up to 255
    document_filename_encrypted = Column(String(384), nullable=False)
    document_filename_hash = Column(String(128), nullable=False, index=True)
    ...
    def __init__(self, user_id: int, original_filename: str,
        ...
        parameters for user ID, original filename, document filename,
        document filesize, document mimetype, collection ID, document
        summary, and thumbnail filename. These attributes are used to
        set the corresponding fields for the document.
        """
    ...
    def document_filename(self):
        return decrypt_str(self.document_filename_encrypted)

    @document_filename.setter
    def document_filename(self, document_filename: str):
        self.document_filename_encrypted = encrypt_str(document_filename)
        self.document_filename_hash = hash_str(document_filename)
    ...
    def document_path(self):
        return os.path.join(cfg.DOCUMENTS_PATH, self.document_filename)

    @property
    def document_url(self):
        return cfg.DOCUMENTS_URL + self.document_filename
...
def after_delete_document(mapper, connection, document: Document):
    """
    Event listener triggered after a document is deleted. It executes
    asyncio tasks to delete the document file and the thumbnail file.
    """
    asyncio.get_event_loop().create_task(delete_document_file(document))

    if document.has_thumbnail:
        asyncio.get_event_loop().create_task(delete_thumbnail_file(document))


async def delete_document_file(document: Document):
    """Deletes the document file."""
    await FileManager.delete(document.document_path)


async def delete_thumbnail_file(document: Document):
