...
async def document_upload(
    ...
    **Hooks:**
    ...
    """
    return await _document_upload(file, request, session, cache, current_user)
...
async def document_upload_to_collection(
    request: Request, file: UploadFile = File(...), collection_id: int = None,
    session=Depends(get_session), cache=Depends(get_cache),
    current_user: User = Depends(auth(UserRole.writer))
) -> DocumentUploadResponse:
    return await _document_upload(
        file, request, session, cache, current_user, collection_id)


async def _document_upload(file: UploadFile, request, session, cache,
