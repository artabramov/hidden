...
class FileManagerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_magic_success(self, aio_open_mock):
        ...
        self.magic_instance.from_buffer.return_value = "text/plain; charset=utf-8"  # noqa E501
    ...
    async def test_magic_octet_stream_then_filetype(
            self, guess_mock, aio_open_mock):
        aio_open_mock.return_value = _AIOReader(b"GIF89a...")

        self.magic_instance.from_buffer.return_value = "application/octet-stream"  # noqa E501
    ...
    @patch("app.managers.file_manager.aiofiles.open")
    @patch("app.managers.file_manager.filetype.guess")
    async def test_magic_exception_then_filetype(self, guess_mock,
                                                 aio_open_mock):
        aio_open_mock.return_value = _AIOReader(b"\xff\xd8\xff\xe0...")
    ...
    @patch("app.managers.file_manager.filetype.guess")
    async def test_filetype_none_then_extension(
            self, guess_mock, guess_type_mock, aio_open_mock):
    ...
    @patch("app.managers.file_manager.mimetypes.guess_type")
    @patch("app.managers.file_manager.filetype.guess")
    async def test_all_fail_returns_none(self, guess_mock, guess_type_mock,
                                         aio_open_mock):
    ...
    @patch("app.managers.file_manager.aiofiles.os.stat", new_callable=AsyncMock)  # noqa E501
    ...
    @patch("app.managers.file_manager.aiofiles.open", side_effect=OSError("open failed"))  # noqa E501
    ...
    @patch("app.managers.file_manager.aiofiles.os.path.isfile", new_callable=AsyncMock)  # noqa E501
    ...
    @patch("app.managers.file_manager.aiofiles.os.makedirs", new_callable=AsyncMock)  # noqa E501
    ...
    @patch("app.managers.file_manager.aiofiles.open")
    @patch("app.managers.file_manager.aiofiles.os.replace", new_callable=AsyncMock)  # noqa E501
    async def test_atomic_write_writes_and_replaces(self, replace_mock,
                                                    aopen_mock):
        written = []
        fh = AsyncMock()
    ...
    @patch("app.managers.file_manager.aiofiles.open")
    @patch("app.managers.file_manager.aiofiles.os.replace", new_callable=AsyncMock)  # noqa E501
    async def test_atomic_write_write_error_no_replace(self, replace_mock,
                                                       aopen_mock):
        fh = AsyncMock()
    ...
    @patch("app.managers.file_manager.aiofiles.open")
    @patch("app.managers.file_manager.aiofiles.os.replace", new_callable=AsyncMock)  # noqa E501
    async def test_atomic_write_flush_error_no_replace(self, replace_mock,
                                                       aopen_mock):
        fh = AsyncMock()
        fh.write = AsyncMock()
    ...
    @patch("app.managers.file_manager.aiofiles.os.makedirs", new_callable=AsyncMock)  # noqa E501
    async def test_upload_streams_in_chunks(self, makedirs_mock,
                                            atomic_write_mock):
        file_mock = AsyncMock()
    ...
    async def test_delete_existing_runs_shred(self, create_proc_mock,
                                              isfile_mock):
        isfile_mock.return_value = True
        proc = AsyncMock()
    ...
    @patch("app.managers.file_manager.aiofiles.open")
    @patch("app.managers.file_manager.aiofiles.os.makedirs", new_callable=AsyncMock)  # noqa E501
    async def test_copy_streams_and_atomic(self, makedirs_mock, aopen_mock,
                                           atomic_write_mock):
        src_fh = AsyncMock()
    ...
    @patch("app.managers.file_manager.aiofiles.open")
    @patch("app.managers.file_manager.aiofiles.os.makedirs", new_callable=AsyncMock)  # noqa E501
    async def test_copy_src_open_error_propagates(self, makedirs_mock,
                                                  aopen_mock):
        ok_ctx = AsyncMock()
